...
{
    [TestClass]
    public class OrderedHashSetTest
    {
        class Both
        {
            public LinkedList<int> List = new LinkedList<int>();
            public OrderedHashSet<int> Set = new OrderedHashSet<int>();
            ...
            public Both MoveFirst(int item) { if (List.Remove(item)) List.AddFirst(item); Set.MoveFirst(item); Validate(); return this; }
            public Both MoveLast(int item) { if (List.Remove(item)) List.AddLast(item); Set.MoveLast(item); Validate(); return this; }
            public Both MoveBefore(int item, int mark) { if (item != mark && List.Contains(mark) && List.Remove(item)) List.AddBefore(List.Find(mark), item); Set.MoveBefore(item, mark); Validate(); return this; }
            public Both MoveAfter(int item, int mark) { if (item != mark && List.Contains(mark) && List.Remove(item)) List.AddAfter(List.Find(mark), item); Set.MoveAfter(item, mark); Validate(); return this; }
        }

        Both New { get { return new Both(); } }
        Both NoTrim { get { return new Both() { EnableTrim = false }; } }
